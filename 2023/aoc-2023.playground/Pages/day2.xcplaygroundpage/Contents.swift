...
func part1() -> Int
{
    ...
    let reDrawRes = /((?'red'[0-9]+) red)|((?'green'[0-9]+) green)|((?'blue'[0-9]+) blue)/

    var sum = 0
    for line in games.split(separator: "\n") {
        var gameDraws = line.split(separator: ":")

        var possible = true
        for draw in gameDraws[1].split(separator: ";") {
            for match in draw.matches(of: reDrawRes) {
                if let value = match.red {
                    if Int(value)! > Max.red {
                        possible = false
                        break
                    }
                }
                if let value = match.green {
                    if Int(value)! > Max.green {
                        possible = false
                        break
                    }
                }
                if let value = match.blue {
                    if Int(value)! > Max.blue {
                        possible = false
                        break
                    }
                }
        ...
        gameDraws[0].removeFirst(5)
        if let id = Int(gameDraws[0]) {
            if possible {
                sum += id
            }
        }
        else {
            print("Could not parse game number for \(gameDraws[0])")
            return 0
        }
    }
...
func part2() -> Int
{
    ...
    for line in games.split(separator: "\n") {
        ...
        if idx == nil {
            print("Could not parse game draws from \(line)")
            return 0
        }

        var draws = line[line.index(after: idx!)...]
